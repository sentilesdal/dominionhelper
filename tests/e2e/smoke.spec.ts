...
test.describe("Extension smoke test", () => {
  ...
  test("extension ID is a valid 32-character string", async ({
    extensionId,
  }) => {
  ...
  test("side panel renders content", async ({ sidePanelPage }) => {
    ...
    await expect(sidePanelPage.locator(".dh-title")).toHaveText(
      "Dominion Helper",
    );

    // Verify both tabs exist (Kingdom and Tracker)
    await expect(sidePanelPage.locator(".dh-tab")).toHaveCount(2);
    await expect(sidePanelPage.locator(".dh-tab").first()).toHaveText(
      "Kingdom",
    );
    ...
    await expect(
      sidePanelPage.locator("#kingdom-tab .dh-empty-state"),
    ).toBeVisible();
    await expect(
      sidePanelPage.locator("#kingdom-tab .dh-empty-state"),
    ).toHaveText("Waiting for a game on dominion.games...");
  });
