...
type AngularElementLike = {
  injector?: () => AngularInjectorLike | undefined;
  scope?: () => RootScopeLike | undefined;
};

type AngularInjectorLike = {
  get: (name: string) => unknown;
};

type AngularLike = {
  element: (target: unknown) => AngularElementLike;
};

type StartGameButtonLike = {
  text?: string;
  canvas?: {
...
type QuestionLike = {
  buttons?: StartGameButtonLike[];
};

type GameServiceLike = {
  questionModel?: {
    openQuestions?: QuestionLike[];
  };
};

type PlayerServiceLike = {
  getMyName?: () => string | null | undefined;
...
export async function createTableWithBot(page: Page): Promise<void> {
  ...
  await page.waitForFunction(
    () => {
      try {
        const dominionWindow = window as WindowWithAngular;
        const injector = dominionWindow.angular
          ?.element(document.body)
          .injector?.();
        const game = injector?.get("game") as GameServiceLike | undefined;
        const qm = game?.questionModel;
        if (!qm?.openQuestions?.length) return false;
        const oq = qm.openQuestions[0];
        const btn = oq.buttons?.find(
          (button: StartGameButtonLike) => button.text === "Start Game",
        );
        if (btn?.canvas) {
...
export async function getKingdomCardCount(page: Page): Promise<number> {
  ...
  return page.evaluate(
    (base) => {
      ...
      let nameEls = document.querySelectorAll(
        "kingdom-viewer .name-layer .text-fitter-node",
      );
      // Fall back to card-stacks if kingdom-viewer has no results
      if (nameEls.length === 0) {
        nameEls = document.querySelectorAll(
          ".card-stacks .name-layer .text-fitter-node",
        );
      }
      let kingdomCount = 0;
    ...
    .poll(
      async () => {
        const info = await page.evaluate(() => {
          ...
          return {
            turn: parseInt(lastMatch[1], 10),
            player: lastMatch[2].trim(),
          };
        });
...
export async function getPlayerName(page: Page): Promise<string> {
  ...
  const nameFromScope = await page.evaluate(() => {
    const dominionWindow = window as WindowWithAngular;
    if (!dominionWindow.angular) return null;

    const injector = dominionWindow.angular.element(document.body).injector?.();
    if (!injector) return null;

    try {
      const playerService = injector.get("playerService") as
        | PlayerServiceLike
        | undefined;
    ...
    try {
      const rootScope = dominionWindow.angular.element(document).scope?.();
      return (
