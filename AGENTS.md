...
- `docs/workflows/` — Tracker and debugging workflows
...
All code changes follow this workflow — see `.Codex/rules/` for full details:

1. **Worktree**: Create a git worktree (`git worktree add .Codex/worktrees/<name> -b <branch>`) — never commit to main directly
...
4. **Verify**: `npm run typecheck && npm run lint && npm run format:check && npm test && npm run build` — all must pass
5. **PR**: Commit, push, create PR with summary and test plan
6. **Clean up**: Remove the worktree after PR is created

Use the tracked `implement` workflow in `.agents/skills/implement/SKILL.md` for end-to-end changes.
...
npm run test:smoke    # Run smoke Playwright coverage
npm run test:auth     # Run authenticated Playwright coverage
npm run test:login    # Run focused login Playwright debug coverage
...
Behavioral rules are in `.Codex/rules/`:
...
- `land` — PR landing and cleanup workflow
