...
```bash
git worktree add .Codex/worktrees/<name> -b <branch-name>
```

All subsequent file operations must happen inside `.Codex/worktrees/<name>/`.
...
- Follow the TypeScript conventions in `.Codex/rules/code-style.md`

## 4. Write Tests

- Create or update Vitest files in `tests/unit/`
- Add or update Playwright specs in `tests/e2e/` when browser automation is part of the change
